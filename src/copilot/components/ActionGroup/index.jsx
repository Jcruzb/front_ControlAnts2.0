...
function ActionGroup({ block, onAction, pendingActionId, state = "ready", error }) {
  ...
  return <section className={`min-w-0 ${layout}`} aria-label="Siguientes pasos sugeridos por Nilo"><InlineActions actions={block.actions} onAction={onAction} pendingActionId={pendingActionId} blockId={block.id} /></section>;
}
